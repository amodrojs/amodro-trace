...
(function() {
  ...
  requirejs.s.newContext = function (name) {
    ...
    // Stores the post _options.translate contents for use in top level results.
    context._tracedTranslatedContents = {};
    ...
    context.cacheRead = function(filePath, encoding) {
    ...
    context.load = function (moduleName, url) {
      ...
      if (context._isSupportedBuildUrl(url)) {
        ...
        if (hasProp(context._cachedFileContents, url) &&
          ...
          // evaluate it.
          contents = context.cacheRead(url);

          // If there is a translate filter, run it now.
          if (context.config._options.translate) {
            contents =
                 context.config._options.translate(moduleName, url, contents);
          }

          if (context.config._options.includeContents) {
            context._tracedTranslatedContents[url] = contents;
          }
          ...
          if (hasProp(context.plugins, moduleName)) {
            ...
            if (pluginBuilderMatch) {
              ...
              builderName = context.makeModuleMap(pluginBuilderMatch[3],
                                ...
                                null,
                                true).id;
              contents = context.cacheRead(context.nameToUrl(builderName));
            }
          }
        ...
        try {
          ...
          if (mod) {
            ...
            if (logger && logger.error) {
              logger.error('Cannot optimize network URL, skipping: ' + url);
            } else {
  ...
  function Loader(options) {
    ...
    this.id = id;
    this.require = requirejs.config({ context: id });
    this.getContext().config._options = options || {};
  }
