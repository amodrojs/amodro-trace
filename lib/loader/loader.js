...
(function() {
  ...
  function cacheReadAsync(context, path, encoding) {
    if (lang.hasProp(context._cachedRawText, path)) {
      return new Prom(function (resolve, reject) {
        resolve(context._cachedRawText[path]);
      });
    } else {
      return file.read(path, encoding).then(function (text) {
        context._cachedRawText[path] = text;
        return text;
      });
    }
  }
      ...
      if (url.indexOf('://') === -1 && url.indexOf('?') === -1 &&
        ...
        if (!layer.ignoredUrls[url]) {
          if (url.indexOf('empty:') === -1) {
            console.log('Cannot optimize network URL, skipping: ' + url);
          }
      ...
      if (context._isSupportedBuildUrl(url)) {
        ...
        Prom.resolve().then(function () {
          ...
          if (hasProp(context._cachedFileContents, url) &&
            ...
            // evaluate it.
            return cacheReadAsync(context, url).then(function (text) {
              if (loadStatus.canceled) {
                return;
              }
              ...
              if (context.config._options.cjsTranslate &&
                (!context.config.shim ||
                 !lang.hasProp(context.config.shim, moduleName))) {
                contents = commonJs.convert(url, contents);
              }

              // If there is a read filter, run it now.
              if (context.config._options.onBuildRead) {
                contents = context.config
                           .onBuildRead(moduleName, url, contents);
              }
          ...
          if (contents) {
            __exec(contents, context.require, makeDefine(context));
          }
  ...
  // TODO, get this error up to callers.
  ...
  if (!requirejs.onResourceLoad) {
    ...
    requirejs._readFile = function(filePath, encoding) {
      return fs.readFileSync(filePath, encoding || 'utf8');
    };

    requirejs._fileExists = function (filePath) {
  ...
  var loader = {
    ...
    create: function(options, id) {
      ...
      var instance = requirejs.config({ context: id });
      loader.getContext(id).config._options = options || {};
    ...
    discard: function(id) {
      delete requirejs.s.contexts[id];
    }
  };
